...
{
    use ApiResponse;
    public function store(Request $request, PowerService $powerService)
    {
        $user = auth('api')->user();
        if (!$user) {
        ...
        DB::transaction(function () use ($student, $test, $request, $powerService, &$result) {
            $result = $powerService->calculatePoints($student, $request->distance);
